...
class DataLoader(object):
    def __init__(
        ...
        for t in self.timepoints:
            corr_df = self.tf_correlation_dfs[t]

            # Pass the 2D correlation array directly so SciPy uses Euclidean distance
            X = corr_df.fillna(0.0).values
            self.linkage_matrices[t] = linkage(X, method="ward")

            # Count the baseline number of clusters
            self.baseline_k[t] = self.tf_louvain[t].nunique()
    ...
    def load_tf_correlations(self):
        ...
        for time in self.timepoints:
            corr_dfs[time] = pd.read_csv(
                join(self.base_path, f"tf_tf_{time}.csv"), index_col=0
            )
        return corr_dfs

    def get_parcat_dimensions(self):
        self.dimensions = [
            dict(values=self.tf_louvain[label], label=label)
            for label in self.timepoints
        ]
    ...
    def get_out_edges_dataframe(self, tf_name, attr_name, threshold=0.1):
        ...
        for time in self.timepoints:
            ...
            for gene in index:
                values.append(
                    self.networks[time].get_edge_data(tf_name, gene)[attr_name]
                )
            data_df[time] = values
    ...
    def get_in_edges_dataframe(self, gene_name, attr_name, threshold=0.05):
        ...
        for time in self.timepoints:
            ...
            for tf_name in index:
                values.append(
                    self.networks[time].get_edge_data(tf_name, gene_name)[attr_name]
                )
            data_df[time] = values
        data_df["avg"] = data_df.mean(axis=1)
